...
<li class="pure-menu-item">
  <button class="pure-menu-link pure-button-primary" @click=${()=>e.onDone()}>Apply filter
      ...
      ${a?function(e){return R`
<li class="pure-menu-item">
  <input class="pure-menu-link" type="text" placeholder="Regular expression" @input=${function(t){e.filterSearch=t.target.value,xe()}}>
</li>`}(a):""}
...
<li class="pure-menu-item">
  ...
  <tbody>
    ${t.tableBody.map(function(e,n){const{editable:i,keydown:s,render:r}=n===V?P(t):j;return R`<tr @dblclick=${function(e){V=n,t.redrawPage(),e.target.focus()}}>${e.map(function(e){return R`<td ?contenteditable=${i} @keydown=${s}>${r(""+e)}</td>`})}</tr>`})}
  </tbody>
</table>`}const J=()=>R``,L=new Event("Item removed"),G=(e,t,n)=>R`
  ...
  ${s}
...
</fieldset>`;function ne(e){if(!1===e.display)return J;if(e.customEditor)return e.customEditor;switch(e.type){case"object":return W(ee(e))(t(e));case"array":const n=e;if("string"===n.items.type){const t=n.items;if(!t.enum&&!t.pattern&&!t.readOnly)return W(X(e))(H)}return function(e){const n=e.items;let i=1;const s=e=>te((()=>n.title?n:Object.assign({},n,{title:n.title||"#"+i++}))(),e);return W(ee(e))(r(function e(n){switch(n.type){case"object":const i=t(n);return e=>W(s(e))(i);case"array":const a=r(e(n.items));return e=>W(s(e))(a);default:const o=ie(n);return e=>W(Z(e))(o)}}(n)));function r(e){const t=[];return(i,s,r,a)=>{return R`
        <fieldset>
          ${(i||[]).map(function(n,i,o){t.length<=i&&(t[i]=e(()=>(function(e,n){e.splice(n,1),t.splice(n,1),s(e),r(L),a()})(o,i)));return t[i](n,e=>(function(e,t,n){e[t]=n,s(e)})(o,i,e),r,a)})}
          <div class="pure-control-group">
            <label></label>
    ...
    <div class="pure-button-primary pure-button"
      ...
      <div id="main">
        ...
        <div class="content" @click=${()=>Ne&&t()}>${function(){switch(e.type){case"action":return Ie(e.action);case"meta":return me(e);case"table":default:return C(e)}}()}</div>
      </div>
    </div>
`,document.body)}const Ee=e=>Ne?e+" active":e;function Ie({text:e,click:t}){return R`<button class="pure-button pure-button-primary" @click=${t}>${e}</button>`}const xe=()=>Te(be);gapi.load("client:auth2",()=>{gapi.client.init({apiKey:"AIzaSyAXtETV2s9eaSPC06WVCW3u1eZku0Uh8ic",discoveryDocs:["https://people.googleapis.com/$discovery/rest?version=v1","https://www.googleapis.com/discovery/v1/apis/fusiontables/v2/rest"],clientId:"340312358145-7kum53te698am4dlopel4ea6j8fqqq79.apps.googleusercontent.com",scope:["profile https://www.googleapis.com/auth/fusiontables","https://www.googleapis.com/auth/fusiontables.readonly"].join(" ")}).then(()=>{gapi.auth2.getAuthInstance().isSignedIn.listen(Oe),Oe(gapi.auth2.getAuthInstance().isSignedIn.get())})});const Oe=e=>e?we(location.hash):Te(fe);function we(e){Te(Ae(s(e)))}function Ae(e){const{tableId:n,limit:i,orderBy:s,addFilter:r,meta:a}=e;if(n){return async function(e,t){const[n,i,s,l]=await Promise.all([gapi.client.fusiontables.table.get({tableId:e}),c("select * "+t),c("select ROWID "+t),function(){const t=o(r);return r?c(`select '${r}', count() from ${e}${t} group by '${r}'`):{}}()]);function c(e){return gapi.client.fusiontables.query.sql({sql:e})}Te(a?_e(n.result,a):ye(n.result,i.result,s.result,r,r?l.result:void 0))}(n,`from ${n}${o()}${s?" order by "+s:""} limit ${i||30}`),ge;function o(n){const i=t(e.filter,n);return i.length?" where "+i.join(" and "):""}}return async function(){const e="show tables",t=await gapi.client.fusiontables.query.sql({sql:e});Te(ve(e,t.result))}(),ge}window.addEventListener("hashchange",()=>we(location.hash)),Te(be);
